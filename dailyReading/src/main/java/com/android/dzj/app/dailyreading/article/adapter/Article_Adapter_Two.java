...
public class Article_Adapter_Two extends BaseAdapter {

	private Context mContext;
	private List<Article_Inof> mArticleList ;
	...
	public class ListViewItem {
		...
		private void setData(int position) {
			...
			Article_Inof info = mArticleList.get(position);
...
//				ImageLoader.getInstance().displayImage(info.contentImg, Iv_contentImg, new SimpleImageLoadingListener());
	...
	public Article_Adapter_Two(Context context, List<Article_Inof> mainList) {
		this.mContext = context;
		this.mArticleList = new ArrayList<Article_Inof>();
		setList(mainList);
	...
	public void setList(List<Article_Inof> mainList)
	{
		...
		this.mArticleList.addAll(mainList);
	...
	public Object getItem(int position) {
		return mArticleList == null ? null : mArticleList.get(position);
	}

	@Override
	public long getItemId(int position) {
		return mArticleList == null ? 0 : position;
	}
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		ListViewItem item;
		if(convertView == null) {
			convertView = inflater.inflate(R.layout.article_item_two, null);
